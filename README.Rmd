...
```{r xtable,result="asis",echo=FALSE, comment=NA }
library("xtable")
 colNamesTable = c("Postgres","MongoDB")
 rowNamesTable = c("Importowanie danych", "Średnia danych w sekundach")
 row1 = c("6.10 m" ,"6.42 m")
 mean1 = c("16487 rekordów", "15665 rekordów")
 m = t(matrix(c(row1,mean1),2,2))
 rownames(m) = rowNamesTable
 colnames(m) = colNamesTable
 #row2 = c("19.97 s","0 s" )
 #row3 = c("19.85 s", "46.4 s")
 d = xtable(m,align = "lrp{3in}") 
 print(m,type='markdown', quote=FALSE)
